...
const users = testData.users
  .map(x => `(${x.id}, '${x.email}', '${x.password}')`)
  .join(', ');
const usersInfo = testData.users
  .map(
    x =>
      `(${x.id}, '${x.name}', '${x.birthday}', ${x.gender}, ${
        x.location.lat
      }, ${x.location.long}, '${x.bio}')`
  )
  .join(', ');
const education = testData.users
  .map(x => `(${x.id}, ${x.education})`)
  .join(', ');
const pictures = testData.users
  .map((x, i) => `(${i + 1}, ${x.id}, '${x.imgs[0]}', 1)`)
  .join(', ');
...
const matches = testData.matches
  .map(x => `(${x.user1}, ${x.user2}, '${x.action}', '${x.date}')`)
  .join(', ');
