...
mysql
  ...
  .then(conn => {
    ...
    const res = conn.query(
      'CREATE USER IF NOT EXISTS ${MYSQLDB.user}@${MYSQLDB.host} IDENTIFIED BY ${MYSQLDB.password}; GRANT ALL PRIVILEGES ON ${MYSQLDB.database}.* TO ${MYSQLDB.user}@${MYSQLDB.host} WITH GRANT OPTION;'
    );
    conn.end();
    return res;
  })
